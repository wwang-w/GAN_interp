...
if __name__=="__main__":
    ...
    parser.add_argument('--model', default='WGAN',  help='')
    parser.add_argument('--train_data', default='/home/wj/DL_inter/test_mat/data/train', help='path to trn dataset')
    parser.add_argument('--val_data', default='/home/wj/DL_inter/test_mat/data/val', help='path to val dataset') 
    ...
    parser.add_argument('--rate', type=int, default=2,help = 'the sampling rate of data')
    ...
    parser.add_argument('--wd', type=float, default=0.00004, help='weight decay, default=0.0004')
    ...
    generator_config = {"imageSize": opt.imageSize,'inputChannel':inputChannel, 'outputChannel':outputChannel, 'ngf':ngf,'rate':rate}
    ...
    if opt.Gmodel == 'mlp_D':
        ...
        netD = net.D(imageSize,inputChannel, ndf)
    ...
    val_target_cpu, val_input_cpu ,val_mask_cpu = val_target_original*opt.G, val_input_original*opt.G,val_mask_original*opt.G
    ...
    if opt.rmsprop:
        ...
        optimG = optim.Adam(netG.parameters(), lr = opt.lrG, betas = (opt.beta1, 0.999), weight_decay=0.0)  

    initial_epoch = findLastCheckpoint(save_dir=opt.exp)  # load the last model in matconvnet style
    if initial_epoch > 1:
        print('resuming by loading epoch %03d' % initial_epoch-1)
        # model.load_state_dict(torch.load(os.path.join(save_dir, 'model_%03d.pth' % initial_epoch)))
        netG.load_state_dict(torch.load(os.path.join(opt.exp, 'netG_epoch_{0}.pth'.format(initial_epoch-1))))
        netD.load_state_dict(torch.load(os.path.join(opt.exp, 'netD_epoch_{0}.pth'.format(initial_epoch-1))))
    else:
        initial_epoch = 0

    schedulerD = MultiStepLR(optimD, milestones=[40, 80,120], gamma=0.5)
    schedulerG = MultiStepLR(optimG, milestones=[40, 80,120], gamma=0.5)
    ...
    for epoch in range(initial_epoch,opt.epoch):
        ...
        while i < len(dataloader):
            ...
            # compute MSE_loss
            err_MSE_ = criterionMSE((1-mask)*x_hat,(1-mask)*target)
